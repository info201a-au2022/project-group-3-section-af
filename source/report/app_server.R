...
ggplot(data = remove_missing(dz_1, na.rm = TRUE,
                             vars = dz_1$race), aes(x = Date, y = race, fill = race, na.rm = TRUE)) +
...
ggplot(remove_missing(dz_1, na.rm = TRUE,
                     vars = dz_1$gender), aes(x = Date, y = gender, fill = gender, na.rm = TRUE)) +
  geom_density_ridges() +
  theme_ridges() +
  theme(legend.position = "none")
...
server <- function(input, output) {
  ...
  output$wave <- renderPlot ({
    if(input$raceorsex == "Sex") {
      ggplot(data = dz_1, aes(x = Date, y = gender, fill=gender)) +
        geom_density_ridges() +
        theme_ridges() +
        theme(legend.position = "none")
    }
    else if (input$raceorsex == "Race") {
      ggplot(data = remove_missing(dz_1, na.rm = TRUE,
                                   vars = dz_1$race), aes(x = Date, y = race, fill = race, na.rm = TRUE)) +
        geom_density_ridges(stat="binline", bins=10) +
        theme_ridges() +
